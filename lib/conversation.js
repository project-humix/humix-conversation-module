...
Conversation.prototype.initMic = function () {
    ...
    var micConfig = {
        'rate': self.config.stt.sttSampleRate || 16000,
        'channels': self.config.stt.sttChannels || 1,
        'debug': self.config.stt.sttDebug || false,
        'exitOnSilence': self.config.stt.sttSilenceInterval || 30
    };

    logger.info('using mic config:'+JSON.stringify(micConfig));
    micInstance = mic( micConfig, logger);

    micInputStream = micInstance.getAudioStream();

    if(self.config.debug)
       micInputStream.pipe(new wav.FileWriter('stt_debug.wav'));
    ...
    micInputStream.on('startSpeech', function() {
        ...
        if(fsm.current != 'suspend') {
            ...
            if (fsm.current === 'sleep') {
                self.sttEngine.reconnect();
            }
